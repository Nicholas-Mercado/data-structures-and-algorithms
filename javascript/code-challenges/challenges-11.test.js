...
const count = (target, input) => {
  let newTarget = input.flat();
  return newTarget.filter(x => x === target).length;
...
const totalSum = (input) => {
  let target = input.flat();
  let final = target.reduce((x , y) => x + y);
  return final;
};
...
let findShortest = (data) => {
 return data.reduce((shortestNow, char) => +shortestNow.height < +char.height ? shortestNow : char).name;
};
